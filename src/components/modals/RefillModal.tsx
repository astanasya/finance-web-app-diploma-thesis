...
import { refillGoal } from "@/actions/goals";
...
export function RefillModal({ goal, accounts, onClose }: any) {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    const res = await refillGoal(goal.id, selectedAccount, parseFloat(amount));
    setLoading(false);
    if (res.success) onClose();
  };

  // Стилі для синхронізації з іншими модалками
  const labelStyles = "text-gray-400 text-[10px] font-medium tracking-widest ml-1";
  const inputStyles = "bg-transparent border-0 border-b border-gray-800 rounded-none focus-visible:ring-0 focus-visible:border-primary-500 px-1 text-base placeholder:text-gray-700 h-10 shadow-none w-full transition-all";

  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="bg-[#161617] border border-white/5 text-white p-10 rounded-[32px] sm:max-w-[460px]">
        <DialogHeader className="mb-8">
          <DialogTitle className="text-3xl font-bold text-center tracking-tight">Refill the goal</DialogTitle>
          <p className="text-zinc-500 text-center text-xs mt-2">Please fill out the form below</p>
        </DialogHeader>

        <div className="space-y-10">
          {/* REFILL FROM (ACCOUNTS CHIPS) */}
          <div className="space-y-3">
            <Label className={labelStyles}>Refill from</Label>
